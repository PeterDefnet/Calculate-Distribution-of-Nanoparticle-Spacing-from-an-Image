...
def plot_neighbor_histogram(distances, count, scale_bar = 1, scale_bar_unit = "Pixels"):
    ...
    with warnings.catch_warnings(): #Ignore warnings given by 'blob_log'
        ...
        plt.title("Euclidean Distance of Direct Neighbors", fontsize=11)
        plt.xlabel("Euclidean Distance / " + str(scale_bar_unit))
        plt.ylabel("Density")

        #Overlay mean, median, std as immediate readout. Else could output in txt. 
        plt.text(60 * scale_bar, 0.055 / scale_bar, "Scale, " + scale_bar_unit + "/Pixel" + " = " + str(scale_bar), fontsize = 9)
        plt.text(60 * scale_bar, 0.05 / scale_bar, "Mean = " + str(round(float(np.mean(distances)), 1)) + " " + str(scale_bar_unit), fontsize = 9)
        plt.text(60 * scale_bar, 0.045 / scale_bar, "Median = " + str(round(float(np.median(distances)), 1)) + " " + str(scale_bar_unit), fontsize = 9)
        plt.text(60 * scale_bar, 0.04 / scale_bar, "Stdev = " + str(round(float(np.std(distances)), 1)) + " " + str(scale_bar_unit), fontsize = 9)
        plt.text(60 * scale_bar, 0.035 / scale_bar, "Total # NP's = " + str(count), fontsize = 9)
        plt.text(60 * scale_bar, 0.030 / scale_bar, "Total # Distances = " + str(len(distances)), fontsize = 9)
